...
window.checkNotificationStatus = function() {
    ...
    if (!("Notification" in window)) { 
        statusText.innerText = "Brak wsparcia dla powiadomień."; 
        btn.classList.add('hidden'); 
        return; 
    }
    
    if (Notification.permission === "granted") { 
        statusText.innerText = "Aktywne 🔔"; 
        btn.innerText = "Odśwież"; 
        btn.classList.remove('hidden'); 
    } else if (Notification.permission === "denied") { 
        statusText.innerText = "Zablokowane 🔕"; 
        btn.classList.add('hidden'); 
    } else { 
        statusText.innerText = "Wymaga zgody"; 
        btn.innerText = "Włącz"; 
        btn.classList.remove('hidden'); 
    }
};
...
window.requestNotificationPermission = async function() {
    ...
    try {
        ...
        if (permission === "granted") {
            ...
            if (!subscription) { 
                subscription = await registration.pushManager.subscribe({ userVisibleOnly: true, applicationServerKey: window.urlB64ToUint8Array(CONFIG.VAPID_PUBLIC_KEY) }); 
            }
...
window.fetchRoomsFromDB = async function() {
    const { data } = await window.supabaseClient.from('rooms')
        .select('*')
    ...
    listEl.innerHTML = appRooms.map(room => `
        <div class="flex justify-between items-center px-3 py-2 bg-[#1e1f20] rounded-[16px] border border-[#333537] mb-1.5">
            ...
            <div class="flex gap-1">
                <button onclick="window.openEditRoomModal('${encodeURIComponent(room.name)}', '${room.icon}')" class="w-8 h-8 rounded-full flex items-center justify-center text-neutral-400 hover:bg-[#333537] hover:text-neutral-200 transition-colors text-sm">✏️</button>
                <button onclick="window.deleteRoom('${encodeURIComponent(room.name)}')" class="w-8 h-8 rounded-full flex items-center justify-center text-neutral-400 hover:bg-[#3c1414] hover:text-[#ffb4ab] transition-colors text-sm">🗑️</button>
            </div>
        </div>
    `).join('');
};
...
window.saveNewRoom = async function() {
    ...
    const { error } = await window.supabaseClient.from('rooms').insert([{ 
        name: name, 
        icon: icon || '📦', 
    ...
    if (error) { 
        if (error.code === '23505') window.showToast('Istnieje!'); 
...
window.saveEditRoom = async function() {
    ...
    const { error } = await window.supabaseClient.from('rooms')
        ...
        return `
            <div class="flex justify-between items-center px-3 py-2 bg-[#1e1f20] rounded-[16px] border border-[#333537] mb-1.5">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <span class="text-sm font-medium text-neutral-200 flex items-center">${window.esc(p.name)} ${window.getAgeBadge(p.birth_date)}</span>
                ...
                <button onclick="window.openEditProfileScreen(${p.id})" class="w-8 h-8 rounded-full flex items-center justify-center text-neutral-400 hover:bg-[#333537] hover:text-neutral-200 transition-colors text-sm">⚙️</button>
            </div>`;
    }).join('');
};
...
window.saveProfileDetails = async function() {
    ...
    const { error } = await window.supabaseClient.from('profiles')
        .update({ name: name, birth_date: birth, height: height, weight: weight })
        .eq('id', id)
        .eq('household_id', window.currentUser.household_id);
...
window.saveTaskSettings = async function() {
    ...
    if (window.currentEditingHomeTask !== n) {
        await window.supabaseClient.from('activity_logs')
            .update({ activity_name: n })
        ...
        await window.supabaseClient.from('tasks')
            .delete()
            .eq('name', window.currentEditingHomeTask)
            .eq('household_id', hid);
            
        window.currentEditingHomeTask = n;
    } else {
        const { error } = await window.supabaseClient.from('tasks')
            .update({ interval_days: i, remind_days_before: remind, push_enabled: pushEnabled, show_in_history: showHist, room: r })
            .eq('name', window.currentEditingHomeTask)
            .eq('household_id', hid);
...
window.permanentlyDelete = function(table, id) {
    window.customConfirm("Usunąć trwale? Tej operacji nie można cofnąć, usunie również historię wpisów!", async () => {
        if (table === 'tasks') {
            const { data } = await window.supabaseClient.from('tasks')
                .select('name')
                .eq('id', id)
                .single();
