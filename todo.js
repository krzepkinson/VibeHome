    ...
    if (lists.length > 0) {
        ...
        html += lists.map(list => `
            <div onclick="openChecklistScreen(${list.id}, '${window.esc(list.title)}')" class="flex items-center justify-between p-3 bg-[#004a77]/20 rounded-[16px] border border-[#004a77]/50 mb-1.5 cursor-pointer active:scale-95 transition-transform shadow-sm">
                <div class="flex items-center gap-3"><span class="text-lg">🗂️</span><span class="text-sm font-medium text-[#c2e7ff]">${window.esc(list.title)}</span></div>
...
window.toggleTodo = async function(id, currentStatus) {
    ...
    if (!currentStatus) updateData.completer_name = window.currentUser.name; 
    else updateData.completer_name = null; 
...
window.loadChecklistItems = async function() {
    const listEl = document.getElementById('checklist-items-list'); listEl.innerHTML = `<p class="text-neutral-500 text-xs text-center py-10">Ładowanie...</p>`;
...
window.toggleChecklistItem = async function(id, currentStatus) {
    await supabaseClient.from('checklist_items').update({ is_completed: !currentStatus }).eq('id', id).eq('household_id', window.currentUser.household_id); loadChecklistItems();
};

window.deleteChecklistItem = async function(id) {
    await supabaseClient.from('checklist_items').delete().eq('id', id).eq('household_id', window.currentUser.household_id); loadChecklistItems();
};
