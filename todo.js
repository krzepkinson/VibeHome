...
async function loadTodosAndLists() {
    ...
    if (lists.length > 0) {
        ...
        html += lists.map(list => `
            <div onclick="openChecklistScreen(${list.id}, '${window.esc(list.title)}')" class="flex items-center justify-between p-3 bg-[#004a77]/20 rounded-[16px] border border-[#004a77]/50 mb-1.5 cursor-pointer active:scale-95 transition-transform shadow-sm">
                <div class="flex items-center gap-3 min-w-0"><span class="text-lg shrink-0">🗂️</span><span class="text-sm font-medium text-[#c2e7ff] truncate">${window.esc(list.title)}</span></div>
                <button onclick="event.stopPropagation(); archiveChecklist(${list.id})" class="text-neutral-500 hover:text-[#ffb4ab] px-2 text-sm shrink-0">🗑️</button>
            </div>`).join('');
    ...
    if (todos.length === 0) {
        ...
        html += todos.map(todo => {
            ...
            let currentName = isDone ? (todo.completer_name || '?') : (todo.creator_name || '?');
            let initial = currentName[0].toUpperCase();
            let badgeType = isDone ? 'todos' : 'todos_creator';
            
            let userBadge = `<div onclick="event.stopPropagation(); window.openChangeUserModal('${badgeType}', ${todo.id}, '${window.esc(currentName)}')" class="w-5 h-5 rounded-full ${isDone ? 'bg-[#0f5223]/30 border-[#0f5223]/50 text-[#c4eed0]' : 'bg-[#333537] border-[#444746] text-neutral-300'} border text-[9px] flex items-center justify-center ml-2 shrink-0 cursor-pointer active:scale-90 transition-transform" data-user-name="${window.esc(currentName)}">${initial}</div>`;
            ...
            <div class="flex items-center justify-between p-3 bg-[#1e1f20] rounded-[16px] border border-[#333537] mb-1.5 ${isDone ? 'opacity-50' : ''}">
                <div class="flex items-center gap-2 flex-1 cursor-pointer min-w-0" onclick="openEditTodoModal(${todo.id}, '${window.esc(todo.title)}')">
                    <div onclick="event.stopPropagation(); toggleTodo(${todo.id}, ${isDone})" class="w-6 h-6 rounded-full border-2 ${isDone ? 'bg-[#c4eed0] border-[#c4eed0]' : 'border-[#444746]'} flex items-center justify-center transition-colors shrink-0">
                        ${isDone ? '<span class="text-[#0f5223] text-xs font-bold">✓</span>' : ''}
                    </div>
                    <span class="text-sm truncate ${isDone ? 'line-through text-neutral-500' : 'text-neutral-200'}">${window.esc(todo.title)}</span>
                    ${userBadge}
                </div>
                <button onclick="event.stopPropagation(); archiveTodo(${todo.id})" class="text-neutral-600 hover:text-[#ffb4ab] pl-3 text-sm shrink-0">🗑️</button>
            </div>`;
        }).join('');
    }
...
window.saveNewTodo = async function() {
    ...
    await supabaseClient.from('todos').insert([{ title: title, user_id: window.currentUser.id, household_id: window.currentUser.household_id, is_completed: false, is_archived: false, creator_name: window.currentUser.name }]);
...
window.saveNewList = async function() {
    ...
    await supabaseClient.from('checklists').insert([{ title: title, user_id: window.currentUser.id, household_id: window.currentUser.household_id, is_archived: false }]);
...
window.archiveTodo = async function(id) {
    if (!confirm("Zarchiwizować?")) return;
...
window.archiveChecklist = async function(id) {
    if (!confirm("Zarchiwizować listę?")) return;
...
window.saveEditedTodo = async function() {
    ...
    window.closeEditTodoModal(); loadTodosAndLists();
}
...
window.loadChecklistItems = async function() {
    const listEl = document.getElementById('checklist-items-list'); if (!listEl) return;
    listEl.innerHTML = `<p class="text-neutral-500 text-xs text-center py-10">Ładowanie...</p>`;
    ...
    const items = data || [];
    if (items.length === 0) { listEl.innerHTML = `<p class="text-center text-neutral-500 text-xs py-10">Lista jest pusta.</p>`; return; }
    listEl.innerHTML = items.map(item => `
        <div class="flex items-center justify-between px-3 py-2 bg-[#1e1f20] rounded-[12px] border border-[#333537] mb-1 ${item.is_completed ? 'opacity-50' : ''}">
            <div class="flex items-center gap-3 flex-1 cursor-pointer min-w-0" onclick="toggleChecklistItem(${item.id}, ${item.is_completed})">
                <div class="w-5 h-5 rounded-full border-2 ${item.is_completed ? 'bg-[#c4eed0] border-[#c4eed0]' : 'border-[#444746]'} flex items-center justify-center transition-colors shrink-0">${item.is_completed ? '<span class="text-[#0f5223] text-[10px] font-bold">✓</span>' : ''}</div>
            ...
            <button onclick="deleteChecklistItem(${item.id})" class="text-neutral-600 hover:text-[#ffb4ab] px-2 text-sm shrink-0">✕</button>
        </div>`).join('');
};
...
window.toggleChecklistItem = async function(id, currentStatus) { await supabaseClient.from('checklist_items').update({ is_completed: !currentStatus }).eq('id', id).eq('household_id', window.currentUser.household_id); loadChecklistItems(); };
