...
async function loadProfiles() {
    ...
    if (error) { list.innerHTML = `<p class="col-span-2 text-center text-red-400 text-xs py-10">Błąd połączenia.</p>`; return; }

    profiles = data || [];
    if (profiles.length === 0) { list.innerHTML = `<p class="col-span-2 text-center text-neutral-500 text-xs py-10">Brak profili.</p>`; return; }

    list.innerHTML = profiles.map(p => `
        <button onclick="openProfile(${p.id}, '${encodeURIComponent(p.name)}')" class="bg-[#1e1f20] p-5 rounded-[28px] border border-[#333537] flex flex-col items-center justify-center gap-3 active:scale-95 transition-transform hover:bg-[#333537]/30">
            <div class="w-14 h-14 bg-[#444746] text-neutral-200 rounded-full flex items-center justify-center text-xl font-medium">${p.name.charAt(0).toUpperCase()}</div>
            <span class="font-medium text-sm text-neutral-200">${p.name}</span>
        </button>
    `).join('');
}
...
function renderMonthlyView(container, year, month) {
    let html = `
        <div class="grid grid-cols-7 text-center text-[9px] font-medium text-neutral-500 uppercase tracking-widest mb-3">
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        if (hasEvents) {
            // Szare kółeczko chorego dnia
            cellClass += "bg-[#444746] text-neutral-100 cursor-pointer active:scale-90 font-medium";
        } else {
            cellClass += "text-neutral-400 bg-transparent";
        }

        if (isToday) {
            cellClass += " border border-neutral-300 text-neutral-100 font-medium";
            if (!hasEvents) cellClass += " bg-[#333537]";
        }
        ...
        html += `
            <div class="flex justify-center items-center">
                <div ${onClickEvent} class="${cellClass}">${day}</div>
            </div>
        `;
    }
...
function renderYearlyView(container, year) {
    ...
    for (let m = 0; m < 12; m++) {
        html += `
            <div onclick="zoomToMonth(${m})" class="bg-[#131314] p-2 rounded-2xl cursor-pointer active:scale-95 transition-transform border border-[#333537]">
                <h4 class="text-[8px] font-medium text-center text-neutral-500 uppercase tracking-widest mb-1.5">${monthNames[m].substring(0,3)}</h4>
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            // Ciemny widok małych kwadracików
    ...
    list.innerHTML = enrichedTasks.map(({task, latestLog, isActive}) => {
        ...
        if (isCyclical) {
            ...
            if (isActive) {
                ...
                uiState.bgColor = 'bg-[#3c1414]'; // Google's dark red container
        ...
        return `
            <div class="flex items-center justify-between p-4 ${uiState.bgColor} rounded-[24px] border ${uiState.borderColor} mb-1 transition-colors">
                <div class="flex-1 pr-4">
                    <h3 class="font-medium text-neutral-100 text-sm flex items-center gap-2">
                        ${task.name}
                    </h3>
                    <p class="text-[11px] ${uiState.statusColor} mt-0.5 flex items-center gap-1">
...
async function logHealthAction(taskId) {
    ...
    showToast('Zapisano!'); loadProfileDashboard();
}
...
function openNewHealthTaskModal() {
    ...
    toggleHealthInterval(); document.getElementById('new-health-task-modal').classList.remove('hidden');
}
...
function toggleHealthInterval() {
    ...
    if (type === 'duration') container.classList.add('hidden'); else container.classList.remove('hidden');
}
...
async function saveNewHealthTask() {
    ...
    if (!name) return;
    await supabaseClient.from('health_tasks').insert([{ profile_id: currentProfileId, name: name, task_type: type, interval_days: type === 'cyclical' ? (parseInt(interval) || 0) : 0 }]);
...
function openHealthSettingsScreen(taskId) {
    ...
    if (task.task_type === 'cyclical') {
        intervalContainer.classList.remove('hidden'); document.getElementById('set-h-task-interval').value = task.interval_days;
    } else { intervalContainer.classList.add('hidden'); }
    renderHealthHistory();
...
async function saveHealthTaskSettings() {
    ...
    healthTasks = res.data; document.getElementById('h-settings-title').innerText = name;
}
...
function renderHealthHistory() {
    ...
    container.innerHTML = logs.map(l => {
        ...
        if (task.task_type === 'duration') {
            ...
            dateText = `<span class="text-[9px] uppercase tracking-widest text-neutral-500">Od:</span> ${startDate} <span class="text-[9px] uppercase tracking-widest text-neutral-500 ml-1">Do:</span> ${endDate}`;
        }
        const encodedNotes = encodeURIComponent(l.notes || '');
        const safeStart = l.start_date; const safeEnd = l.end_date || 'null';

        return `
            <div class="bg-[#131314] p-3 rounded-[16px] flex justify-between items-center border border-[#333537] mb-2">
                <div>
                    <p class="text-xs font-medium text-neutral-200">${dateText}</p>
                    ${l.notes ? `<p class="text-[10px] text-neutral-400 mt-1">${l.notes}</p>` : ''}
                </div>
                <div class="flex gap-1">
                    <button onclick="openEditHealthLogModal(${l.id}, '${safeStart}', '${safeEnd}', '${encodedNotes}', '${task.task_type}')" class="w-8 h-8 rounded-full flex items-center justify-center text-neutral-400 hover:bg-[#333537] hover:text-neutral-200 transition-colors text-sm">✏️</button>
                    <button onclick="deleteHealthLog(${l.id})" class="w-8 h-8 rounded-full flex items-center justify-center text-neutral-400 hover:bg-[#3c1414] hover:text-[#ffb4ab] transition-colors text-sm">🗑️</button>
                </div>
            </div>`;
    }).join('') || '<p class="text-center py-4 text-neutral-500 text-xs">Brak wpisów w historii.</p>';
}
...
function openEditHealthLogModal(id, startDate, endDate, encodedNotes, taskType) {
    ...
    if (taskType === 'duration') {
        ...
        endContainer.classList.add('hidden'); endInput.value = '';
    }
...
async function saveEditHealthLog() {
    ...
    let updateData = { start_date: `${start}T12:00:00.000Z`, notes: notes || null };
    ...
    } else { updateData.end_date = updateData.start_date; }
    ...
    healthLogs = res.data || []; renderHealthHistory();
}
