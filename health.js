...
window.renderCalendar = function() {
    ...
    if (calendarViewMode === 'month') {
        ...
        for (let d = 1; d <= daysInMonth; d++) {
            ...
            html += `
                <div onclick="window.openDayDetails('${dateStr}')" class="aspect-square flex items-center justify-center rounded-xl cursor-pointer transition-all active:scale-90 ${dayClass}">
...
window.renderHealthTasks = function() {
    ...
    list.innerHTML = healthTasks.map(task => {
        ...
        const statusStr = window.getHealthStatusString(task, activeLog, tLogs);
...
window.startHealthLog = async function(taskId, type) {
    ...
    await window.supabaseClient.from('health_logs').insert([{ health_task_id: taskId, start_date: now, end_date: (type === 'cyclical' || type === 'one_time') ? now : null, user_id: window.currentUser.id, household_id: window.currentUser.household_id, user_name: window.currentUser.name }]);
...
window.closeHealthLog = async function(logId) {
    await window.supabaseClient.from('health_logs').update({ end_date: new Date().toISOString() }).eq('id', logId).eq('household_id', window.currentUser.household_id);
...
window.saveNewHealthTask = async function() {
    ...
    await window.supabaseClient.from('health_tasks').insert([{ 
        profile_id: currentProfileId, name: n, task_type: type, interval_days: interval, remind_days_before: remind, event_date: evDate, show_in_history: true, is_archived: false, user_id: window.currentUser.id, household_id: window.currentUser.household_id 
    }]);
...
window.saveHealthTaskSettings = async function() {
    ...
    await window.supabaseClient.from('health_tasks').update(updateData).eq('id', window.currentHealthSettingsId).eq('household_id', window.currentUser.household_id);
...
window.deleteHealthLog = function(id) {
    window.customConfirm("Usunąć ten wpis z historii?", async () => {
        await window.supabaseClient.from('health_logs').delete().eq('id', id).eq('household_id', window.currentUser.household_id);
...
window.deleteHealthTask = function() {
    window.customConfirm("Zarchiwizować to zdarzenie? Zniknie z głównych widoków.", async () => {
        await window.supabaseClient.from('health_tasks').update({ is_archived: true }).eq('id', window.currentHealthSettingsId).eq('household_id', window.currentUser.household_id);
