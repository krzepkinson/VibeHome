...
window.initHealthModule = async function() {
    ...
    if (currentProfileId) await window.refreshHealthData();
...
window.renderHealthUI = function() {
    ...
    window.renderCalendar(); window.renderHealthTasks();
};
...
window.renderHealthTasks = function() {
    ...
    list.innerHTML = healthTasks.map(task => {
        ...
        if (task.task_type === 'one_time') {
            if (tLogs.length === 0) actionBtn = `<button onclick="window.startHealthLog(${task.id}, 'one_time')" class="w-8 h-8 rounded-full bg-[#0f5223]/20 text-[#c4eed0] font-bold text-base flex items-center justify-center active:scale-90 border border-[#0f5223]/50">✓</button>`;
        } else {
            actionBtn = activeLog 
...
window.startHealthLog = async function(taskId, type) {
    ...
    const { error } = await window.supabaseClient.from('health_logs').insert([{ health_task_id: taskId, start_date: now, end_date: (type === 'cyclical' || type === 'one_time') ? now : null, user_id: window.currentUser.id, household_id: window.currentUser.household_id, user_name: window.currentUser.name }]);
    if (error) { window.showToast("Błąd: " + error.message); return; }
    window.showToast("Zapisano!"); await window.refreshHealthData(); window.renderHealthUI(); if(typeof window.loadDashboardOverview === 'function') window.loadDashboardOverview();
};
...
window.closeHealthLog = async function(logId) {
    ...
    if (error) { window.showToast("Błąd: " + error.message); return; }
    window.showToast("Zakończono"); await window.refreshHealthData(); window.renderHealthUI(); if(typeof window.loadDashboardOverview === 'function') window.loadDashboardOverview();
};
...
window.saveNewHealthTask = async function() {
    const n = document.getElementById('h-task-name').value.trim(); const type = document.getElementById('h-task-type').value;
    ...
    const { error } = await window.supabaseClient.from('health_tasks').insert([{ profile_id: currentProfileId, name: n, task_type: type, interval_days: interval, remind_days_before: remind, event_date: evDate, show_in_history: true, is_archived: false, user_id: window.currentUser.id, household_id: window.currentUser.household_id }]);
    if (error) { window.showToast("Błąd: " + error.message); return; }
...
window.openHealthSettingsScreen = async function(taskId) {
    window.currentHealthSettingsId = taskId; const task = healthTasks.find(t => t.id === taskId);
...
window.saveHealthTaskSettings = async function() {
    ...
    const n = document.getElementById('set-h-task-name').value.trim(); const showHist = document.getElementById('set-h-task-history').checked;
...
window.renderHealthHistory = function() {
    ...
    document.getElementById('h-settings-history-list').innerHTML = logs.map(l => `
        <div class="bg-[#131314] px-3 py-2 rounded-[12px] flex justify-between items-center border border-[#333537] mb-1.5">
            <div><p class="text-xs font-medium text-neutral-200">${new Date(l.start_date).toLocaleDateString('pl-PL')}</p><p class="text-[9px] text-neutral-500">${l.end_date ? 'do ' + new Date(l.end_date).toLocaleDateString('pl-PL') : 'trwa'}</p></div>
