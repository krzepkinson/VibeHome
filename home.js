...
window.deleteLog = async function(id) {
    if(confirm("Usunąć ten wpis?")) { 
        ...
        loadDashboard(); 
        const res = await supabaseClient.from('activity_logs').select('*').eq('activity_name', window.currentEditingHomeTask).eq('user_id', window.currentUser.id).order('created_at', { ascending: false });
        allHomeLogs = res.data || []; renderHistory(); 
    }
};

window.openEditLogModal = function(id, date, notes) {
    document.getElementById('edit-log-id').value = id; document.getElementById('edit-log-date').value = date; document.getElementById('edit-log-notes').value = decodeURIComponent(notes); document.getElementById('edit-log-modal').classList.remove('hidden');
};
